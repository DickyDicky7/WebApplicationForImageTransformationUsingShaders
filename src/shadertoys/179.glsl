...
float size = 6.0; // length of edges of grid
float angle = 0.0;
...
vec3 halftone_dist(vec2 xy, mat2 m) {
    ...
    col = pow(col, vec3(4.5));	// convert texture to linear gamma
    float maxCircleSizeSq = 0.38 * size * size; // max circle size = 0.5 * size

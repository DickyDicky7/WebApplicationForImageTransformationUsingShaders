...
float processLum( vec2 uv, float lum )
{
    ...
    float lum_scale = smoothstep( 1.0, 0.82, lum);
    float power = mix( 10.0, 3.1, lum_scale);
    power = mix(power, 0.8, smoothstep(0.82, 0.66, lum));
    power = mix(power, 0.0, smoothstep(0.66, 0.5, lum));
...
void main(void)
{
    ...
    vec3 outputColor = mix( vec3( 0.0, 0.2, 0.3), vec3(1.0, 1.0, 1.0), color);

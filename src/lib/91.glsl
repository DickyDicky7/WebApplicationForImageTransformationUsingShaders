...
void main() {
    ...
    for (int i = 0; i < num_of_layers; i++) {
        snow_accumulation += generate_snowflake(vTexCoord, float(i), time, wind);
    }

    // Calculate final color
    float snow_intensity = clamp(length(snow_accumulation), 0.0, 1.0);
    vec4 base_color = texture(tex0, vTexCoord);
    ...
    fragColor = mix(base_color, transparency_color, snow_intensity);
}

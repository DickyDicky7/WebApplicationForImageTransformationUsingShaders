...
//  uniform vec3 Color002; // 0.3, 1.0, 0.9
    ...
    void main() {
    int i = int(vec4(vTexCoord * canvasSize, 0.0, 1.0).y) & 31;
//  int i = int(vec4(vTexCoord * canvasSize, 0.0, 1.0).y) & 31;
...
//  float a = float(i) / 32.f;
    ...
    vec2 uv = .5f + (vec4(vTexCoord * canvasSize, 0.0, 1.0).xy - canvasSize.xy * .5f) * vec2(canvasSize.y / canvasSize.x, 1) / canvasSize.y;
//  vec2 uv = .5f + (vec4(vTexCoord * canvasSize, 0.0, 1.0).xy - canvasSize.xy * .5f) * vec2(canvasSize.y / canvasSize.x, 1) / canvasSize.y;
...
//  // stretch the sides out to fill the frame, but leave the middle the correct aspect ratio
    const float r = .65f;
//  const float r = .65f;
    uv.x = mix(uv.x, vec4(vTexCoord * canvasSize, 0.0, 1.0).x / canvasSize.x, 2.f * smoothstep(1.f - r, 1.f + r, abs(1.f - 2.f * vec4(vTexCoord * canvasSize, 0.0, 1.0).x / canvasSize.x)));
//  uv.x = mix(uv.x, vec4(vTexCoord * canvasSize, 0.0, 1.0).x / canvasSize.x, 2.f * smoothstep(1.f - r, 1.f + r, abs(1.f - 2.f * vec4(vTexCoord * canvasSize, 0.0, 1.0).x / canvasSize.x)));

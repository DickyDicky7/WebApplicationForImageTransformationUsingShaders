...
//  //const float diameter       =    0.2;
    ...
    float rotation_ratio = 2.f * rotation_count * pi; float remapTime = mod(speed * time, 2.f * pi) - pi; remapTime *= step(0.f, remapTime); float diameter = 0.2f + 0.07f * (-(cos(2.f * remapTime) - 1.f) / 2.f); vec2 uv = (vec4(vTexCoord * canvasSize, 0.0, 1.0).xy / canvasSize.x - vec2(0.5f, 0.5f * canvasSize.y / canvasSize.x));
//  float rotation_ratio = 2.f * rotation_count * pi; float remapTime = mod(speed * time, 2.f * pi) - pi; remapTime *= step(0.f, remapTime); float diameter = 0.2f + 0.07f * (-(cos(2.f * remapTime) - 1.f) / 2.f); vec2 uv = (vec4(vTexCoord * canvasSize, 0.0, 1.0).xy / canvasSize.x - vec2(0.5f, 0.5f * canvasSize.y / canvasSize.x));

...
//  vec2 uv = vTexCoord; vec3 col = (change_scene) ? texture(tex0, uv).rgb : texture(uploadTex0, uv).rgb; fragColor = vec4(col, 1); if (ratio <= 0.) { return; }
    ...
    ///   Cube params   ///
    ///////////////////////
    uv = gl_FragCoord.xy / canvasSize.x; float aspect = canvasSize.x / canvasSize.y; vec3 viewingNormal = normalize(vec3(uv - vec2(0.5f, 0.5f / aspect), 1)); vec3 boxPos = vec3(0, 0, mix(closest_pos, farthest_pos, pow((ratio - 0.5f) * 2.f, speed_power)));
//  uv = gl_FragCoord.xy / canvasSize.x; float aspect = canvasSize.x / canvasSize.y; vec3 viewingNormal = normalize(vec3(uv - vec2(0.5f, 0.5f / aspect), 1)); vec3 boxPos = vec3(0, 0, mix(closest_pos, farthest_pos, pow((ratio - 0.5f) * 2.f, speed_power)));

...
void main() {
    float size = max(canvasSize.x, canvasSize.y); vec2 xy = (gl_FragCoord.xy - canvasSize.xy * 0.5f) / size * 2.0f; vec3 rayDir = normalize(vec3(xy, 1.0f)); vec2 uv = xy * 0.5f + 0.5f; fragColor = vec4(vec3((nebula(vec3(uv * 5.1f, time * 0.1f) * 0.1f) - 1.0f)), 1.0f);
//  float size = max(canvasSize.x, canvasSize.y); vec2 xy = (gl_FragCoord.xy - canvasSize.xy * 0.5f) / size * 2.0f; vec3 rayDir = normalize(vec3(xy, 1.0f)); vec2 uv = xy * 0.5f + 0.5f; fragColor = vec4(vec3((nebula(vec3(uv * 5.1f, time * 0.1f) * 0.1f) - 1.0f)), 1.0f);
}

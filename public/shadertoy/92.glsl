...
//  float yNorm =      uv.y * 2.0 - 1.0;
    float  bias = cos(yNorm * 3.141592);
//  float  bias = cos(yNorm * 3.141592);
    float randomShift = -24.0 - hash(vec4(vTexCoord * canvasSize, 0.0, 1.0).y) * bias * 08.0;
//  float randomShift = -24.0 - hash(vec4(vTexCoord * canvasSize, 0.0, 1.0).y) * bias * 08.0;
    randomShift =  min (0.0, randomShift + sin(time)  * 16.0  + 16.0);
//  randomShift =  min (0.0, randomShift + sin(time)  * 16.0  + 16.0);
    int  shiftedIndex = int(clamp(    float(nearestIndex) + randomShift, 0.0, 15.0  ));
//  int  shiftedIndex = int(clamp(    float(nearestIndex) + randomShift, 0.0, 15.0  ));
...
//  fragColor = vec4(getColor( shiftedIndex ), 1.0);
    ...
    // @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
//  // @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
    // https://www.shadertoy.com/view/lcdcDs
//  // https://www.shadertoy.com/view/lcdcDs
    // @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
//  // @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@

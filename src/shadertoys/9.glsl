...
// Constants
const float SPEED = 0.29;
const float TURBULENCE = 0.16;
const float ADVECTION = 0.3;
...
float wave(float x) {
    float x0 = x;
    x = nmapu(sin(x * 45.0 - time * 24.0));
    x = pow(x, 24.0); // sharpen
    x /= x0 * 32.0 + 0.125; // falloff
    x *= exp(-x0 * 2.0); // damping
    return x;
}
...
void main() {
    ...
    float sun = min(1.0, pow(L * 1.5, 3.25));
    float sky = min(1.0, pow(L + 0.39, 1.1));

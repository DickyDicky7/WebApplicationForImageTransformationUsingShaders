...
//  // Normalized pixel coordinates (from 0 to 1)
    vec2 uv = gl_FragCoord.xy / canvasSize.y; uv.y = 1.0f - uv.y; vec2 toCenter = vec2(0.5f) - uv; float dis = length(toCenter); float angle = (acos(toCenter.x * 1.0f / dis) / TWO_PI) * 2.f;
//  vec2 uv = gl_FragCoord.xy / canvasSize.y; uv.y = 1.0f - uv.y; vec2 toCenter = vec2(0.5f) - uv; float dis = length(toCenter); float angle = (acos(toCenter.x * 1.0f / dis) / TWO_PI) * 2.f;

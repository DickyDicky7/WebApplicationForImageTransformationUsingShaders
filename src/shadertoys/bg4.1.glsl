...
void main() {
    vec2 uv = (gl_FragCoord.xy - mousePosition.xy) / canvasSize.xy * 2.f; uv.x *= canvasSize.x / canvasSize.y; vec2 q, r; vec3 col1 = vec3(0.f, .9f, .8f); vec3 col2 = vec3(1.f, .6f, .5f); float f = pattern(uv, q, r, 0.1f * time); vec3 c = mix(col1, vec3(0), smoothstep(.0f, .95f, f)); vec3 a = col2 * smoothstep(0.f, .8f, dot(q, r) * 0.6f); c = sqrt(c * c + a * a); fragColor = vec4(c, 1.f);
//  vec2 uv = (gl_FragCoord.xy - mousePosition.xy) / canvasSize.xy * 2.f; uv.x *= canvasSize.x / canvasSize.y; vec2 q, r; vec3 col1 = vec3(0.f, .9f, .8f); vec3 col2 = vec3(1.f, .6f, .5f); float f = pattern(uv, q, r, 0.1f * time); vec3 c = mix(col1, vec3(0), smoothstep(.0f, .95f, f)); vec3 a = col2 * smoothstep(0.f, .8f, dot(q, r) * 0.6f); c = sqrt(c * c + a * a); fragColor = vec4(c, 1.f);
}

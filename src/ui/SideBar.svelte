...
, import.meta.env.VITE_SUPABASE_KEY_PUB);
  ...
  return array.reduce((result: number[][], value: number, index: number) => {
    ...
    let activeTab: string | null = uniforms.length > 0 ? uniforms[0].thisUniformName : null;
    const selectTab = (tab: string|null) => {
        activeTab = tab;
    }
    
</script>
<div class="grid padding menu light-blue2">
    {#each uniforms as {thisUniformName, thisUniformNameJustForDisplay}}
        <button
            class="s12 m6 l3 {activeTab === thisUniformName ? '' : 'surface-dim'}"
            on:click={() => selectTab(thisUniformName)}>
            <span>{thisUniformNameJustForDisplay}</span>
        </button>
    {/each}
</div>
<div class="space yellow2"/>
...
{#each uniforms as {thisUniformName, thisUniformNameJustForDisplay, thisUniformType, thisUniformDefaultValue}, ii (thisUniformName)}
    {#if activeTab === thisUniformName}
    <div class="padding column content yellow2">
                    ...
                    <td>
                        <div class="field label round border">
                            <input
                                ...
                                step="0.01"
                                value={v}
                                on:input={(e) => updateUniform(ii, i, parseFloat(e.currentTarget.value))}
                            />
                            {#if i === 0}
                                <label>x</label>
                            {/if}

...
//  float zoom = zoom_ratio * sign(remapTime - pi / 2.f) * (1.f - abs(cos(remapTime)));
    ...
    ///   Getting the center   ///
    //////////////////////////////
    vec2 uv = gl_FragCoord.xy / canvasSize.x; vec2 center = side * round((uv + move) / side); center -= move; center.y = center.y * canvasSize.x / canvasSize.y; uv = gl_FragCoord.xy / canvasSize.xy;
//  vec2 uv = gl_FragCoord.xy / canvasSize.x; vec2 center = side * round((uv + move) / side); center -= move; center.y = center.y * canvasSize.x / canvasSize.y; uv = gl_FragCoord.xy / canvasSize.xy;

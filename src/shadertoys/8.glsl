...
float remap_noise_tri_erp(const float v) {
    ...
    return abs(fract(v + 0.5) - 0.5) + 0.25;
    #else
    float r2 = 0.5 * v;
    float f1 = sqrt(r2);
...
void main() {
    ...
    vec2 fragCoord = floor(gl_FragCoord.xy / RESOLUTIONFACTOR) * RESOLUTIONFACTOR;
    ...
    finalColor = vec3(0.333 * (finalColor.r + finalColor.g + finalColor.b));
    #endif

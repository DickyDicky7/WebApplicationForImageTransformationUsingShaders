...
void main(void)
{
    ...
    if (uv.x < (sin(time * 1.5) * 0.3) + 0.5) {
        posterizeStepsVec = vec3(posterizeStepsHue, posterizeStepsSaturation, posterizeSteps);
        vec3 posterizedHsv = round(hsv * posterizeStepsVec) / posterizeStepsVec;
        vec3 posterizedRgb = hsv2rgb(posterizedHsv);
        blended = mix(texCol.rgb, posterizedRgb, posterizeAmount);
    } else {
        posterizeStepsVec = vec3(posterizeSteps, posterizeSteps, posterizeSteps);
        vec3 posterized = round(texCol.rgb * posterizeStepsVec) / posterizeStepsVec;
        blended = mix(texCol.rgb, posterized, posterizeAmount);
    }

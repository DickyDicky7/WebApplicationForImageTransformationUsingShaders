...
vec3 colors[8]; // Declare color array with defined size
...
vec3 Quantize(vec3 col) {
    float dist = distance(colors[0], col); // Initial distance set to first color
    vec3 tempColor = colors[0]; // Placeholder color

    for (int i = 1; i < 8; i++) { // Loop through color array
        float currentDist = distance(colors[i], col);
        if (currentDist < dist) { // Update if closer color found
            dist = currentDist;
            tempColor = colors[i];
        }
    }

    return tempColor; // Return closest color
}

void main(void) {
    initializeColors(); // Initialize color values in array
    ...
    fragColor = vec4(col, 1.0); // Display quantized texture
}

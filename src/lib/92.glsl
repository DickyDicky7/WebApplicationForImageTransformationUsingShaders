...
void main() {
    ...
    if (roll || noise_opacity > 0.0) {
        float remapTime = roll ? time : 0.0;
        roll_line = smoothstep(0.3, 0.9, sin(uv.y * roll_size - (remapTime * roll_speed)));
        roll_line *= roll_line * smoothstep(0.3, 0.9, sin(uv.y * roll_size * roll_variation - (remapTime * roll_speed * roll_variation)));
        roll_uv = vec2(roll_line * distort_intensity * (1.0 - vTexCoord.x), 0.0);
    }

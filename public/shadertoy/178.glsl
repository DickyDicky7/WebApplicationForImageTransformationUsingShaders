...
//  // ShaderToy version//  // ShaderToy version
    void main()
    {
    vec2 resolution = canvasSize.xy; resolution.x = resolution.x *0.4; /* vec2 resolution = (297.0, 420.0) */  vec2 screenCoord = vec4(vTexCoord * canvasSize, 0.0, 1.0).xy;
//  vec2 resolution = canvasSize.xy; resolution.x = resolution.x *0.4; /* vec2 resolution = (297.0, 420.0) */  vec2 screenCoord = vec4(vTexCoord * canvasSize, 0.0, 1.0).xy;

    // Get the texture color at the current pixel
//  // Get the texture color at the current pixel
    vec4 texColor = texture(tex0, vec4(vTexCoord * canvasSize, 0.0, 1.0).xy / resolution);
//  vec4 texColor = texture(tex0, vec4(vTexCoord * canvasSize, 0.0, 1.0).xy / resolution);

...
void main() {
    vec2 uv = gl_FragCoord.xy / canvasSize.yy; uv.y = 1.0f - uv.y; float scale = 5.f; vec3 col = vec3(.1f); float n; vec2 q; vec2 r; n = fbmPattern(scale * uv, q, r); vec3 baseCol = basePalette(r.x); vec3 smokeCol = smokePalette(n); col = mix(baseCol, smokeCol, pow(q.y, 1.3f));
//  vec2 uv = gl_FragCoord.xy / canvasSize.yy; uv.y = 1.0f - uv.y; float scale = 5.f; vec3 col = vec3(.1f); float n; vec2 q; vec2 r; n = fbmPattern(scale * uv, q, r); vec3 baseCol = basePalette(r.x); vec3 smokeCol = smokePalette(n); col = mix(baseCol, smokeCol, pow(q.y, 1.3f));
#ifdef DEBUG_PALETTE
    float x = gl_FragCoord.x / canvasSize.x; col = mix(col, basePalette(x), step(abs(uv.y - .03f), .02f)); col = mix(col, smokePalette(x), step(abs(uv.y - .08f), .02f));
//  float x = gl_FragCoord.x / canvasSize.x; col = mix(col, basePalette(x), step(abs(uv.y - .03f), .02f)); col = mix(col, smokePalette(x), step(abs(uv.y - .08f), .02f));
#endif
